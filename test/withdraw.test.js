...
contract("Splitter", (accounts) => {
	...
	it("Should withdraw the amount correctly", async () => {
		...
		let accountTwoStartAmountGas = amountByTwo
			.add(accountTwoStartingBalance)
			.sub(accountTwoWithdrawGasUsed.mul(accountTwoWithdrawGasPrice));
		let accountThreeStartAmountGas = amountByTen
			.add(accountThreeStartingBalance)
			.sub(accountThreeWithdrawGasUsed.mul(accountThreeWithdrawGasPrice));
	...
	it("Should only work if amount is given", async () => {
		...
		await truffleAssert.fails(splitterInstance.withdraw({ from: accountTwo }), null, "invalid BigNumber value");
	});
	...
	it("Should correctly emit the proper event: Transfer", async () => {
		...
		const withdrawReceipt = await splitterInstance.withdraw(amountByTwo, {
			from: accountTwo,
		});
